...
Once the OS detection mechanism identifies that the NIC has data to process several components of OS functionality must be run in accordance with the execution model of the OS.  Specifically device driver code must remove dequeue hardware frames and schedule them for processing by the appropriate network protocol code. This code must ultimately determine the application end point, a port in the case of IP and TCP, and enqueue, encapsulated data,  for application processing.  Ultimately, this can envolve 
