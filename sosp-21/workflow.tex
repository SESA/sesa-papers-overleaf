...
The nature of the workload drives the length of quiescent period and the nature of the work required to service the request (additional components of the diagram will be discussed next).  Network services tends to fall into two broad categories: Open and Closed Loop.   

\subsubsection{Open Loop:}
\label{sec:workflow:openloop}
In an open loop scenario, such as in-memory key-value stores, the external request rate induces an inter-arrival gap that will drive the quiescent period -- longer at lighter loads (lower queries per second (QPS)) and shorter at heavier loads (higher QPS). The arrival rate can largely be considered independent of the time required to service a request. Moreover, providers often set a Service-level Agreement (SLA) target, where some percentage of requests must be completed under a stringent time budget. In addition, there have been a wealth of research in using these SLA headrooms to lower datacenter energy use mainly by decreasing processor speeds~\cite{Dynamo, SmoothOperator, oldi-pegasus, adrenaline, heracles, energyproportion, warehouse-power}.
...
%herefore categorizing system performance into acceptable and unacceptable.

\subsubsection{Closed Loop:}
\label{sec:workflow:closed_loop}
Examples of closed loop workloads include snapshotting a database to a remote server, video streaming or a middle tier service within a data center~\cite{Barroso:2009:DCI:1643608, oldi-study, oldi-pegasus, warehouse-power, energyproportion, WebSearch}. In this case, the total work to be done is a sequence of requests that have an inter-dependency on each other. Specifically, the arrival of the next request depends on how fast it takes to service the current request. From a server's perspective, the quiescent period will be bounded by the time to transmit both the request and the reply, as well as the time on the client to generate the next request.  
In the closed loop setting, one would like the server to complete every request quickly so that the overall time to complete a task is minimized. However, we find that depending on the nature of the workload and the quiescent period, composed of the network transmission times and client servicing time, there still exist interesting opportunities to reduce energy use dependent upon OS structure and careful manual control of the hardware delay mechanisms.
...
Specializing an OS to support the execution of a single application can explore more extreme strategies like the aggressive polling (\cref{sec:workflow:pollio}), given that it need not arbitrate the device or cores and can be programmed and integrated into a single application.  
%% a general purpose must not starve out vs specialized os

\subsubsection{Interrupt Delaying}
\label{sec:workflow:itrdelay}
\input{itr_flowchart}
A common feature of modern high speed NICs is the ability to delay the delivery of interrupt when an event such as packet receive or transmission completion. By manipulating this setting, software can limit the minimum time between interrupts or in other words the maximum rate at which the NIC events can interrupt the processor. The NIC used in our work exposes this mechanism via an Interrupt Throttling (ITR) setting.  We flowchart the algorithm from the NIC's datasheet~\cite{82599} in  Figure~\ref{fig:itr_delay_flowchart}. Software uses the ITR register to configure a delay in 2$\mu$s increments.  If the spacing of events, such as packet reception, is less than  $2{\mu}s \times ITR$ the NIC will delay assertion.  If on the other hand events are sufficiently separated an interrupt will be asserted immediately.   
...
Library OS's that are customized for a specific application and processing model often shed much of the above complexity; both shortening the path and eliminating the privileged scheduling domains\cite{10.1145/2997641,10.1145/2812806,EbbRT}.  Further, they can exploit short-cuts that enable a run-to-completion execution model, including application processing in response to detecting device activity.  As such, we use the library OS to explore the impacts of shorter OS hot-paths customized to running a single network service.  

\subsubsection{Application Processing}
\label{sec:workflow:appproc}
Once the OS has completed protocol processing, the work now transitions to application logic. \textit{Depending on the workload, this may be very simple as in the case of a workload like memcached or it require significant cpu activity as in the case of  an application server written in managed runtime such as nodejs or one that does non-trivial computational work to service a request.} %% needs shortening

As illustrated in figure~\ref{fig:timeline}, during application processing the OS logic may be interleaved.  This work roughly falls into two broad categories, synchronous work done in service of this application request (page-faults, system calls, etc) and asynchronous work not having to do with this request (OS background work, processing of other requests or processes).
...
Figure~\ref{fig:timeline} also shows that at some point during application processing, a reply is generated by OS for transmission.  This can often be handled in an asynchronous fashion depending on the OS semantics. \textit{Where the OS initiates protocol processing and device transmission in parallel with the remaining application request processing logic (eg. book keeping, cleanup and preparation for the next request).}
