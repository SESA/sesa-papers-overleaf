...
A gap exists in the OS and energy management literature for network oriented services.  Our work seeks to start filling it by offering a detailed OS centric study of how the performance and energy tradeoff interacts with an OS.  We focus on three core aspects; 1) the impacts of using DVFS to "slow" processing, 2) the impacts of delaying interrupt detection using NIC features, and 3) the impact and value of specializing OS paths.

Our contributions stem from a detailed and exhaustive experimental study from which we; a) demonstrate that using a combination of the above three techniques the OS has sufficient control to navigate the large space of performance-energy optimization (see \cref{fig:closed_loop_overview,fig:mcd_overview,fig:mcdsilo_overview}), b) slowing down processing interacts with OS code paths and IO detection mechanism in unique and surprising ways (see \cref{sec:workflow:dvfs,sec:closed_loop:poll,sec:mcd:slowprocvssleep,sec:mcdsilo:dvfstradeoff}), c) uniquely using NIC support for delaying interrupts enable the OS to enact energy optimizations (see \cref{sec:workflow:itr,sec:closed_loop:speedup,sec:mcd:slowinos,sec:mcdsilo:ipc}) including  application level request delaying\cite{mootaz,udpm}, d) specializing OS paths for network processing can yield dramatic improvements including the surprising ability to "slow" the processor to save energy while having little to no impact on time (\cref{sec:workflow:osspec,sec:closed_loop:poll,sec:closed_loop:slowtostaybusy,sec:mcd:poll,sec:mcd:slowinos,sec:mcdsilo}).   

In addition to the above we develop a mathematical framework as a starting point for analytical exploration of how the OS, application and workload interact with the hardware.  
% potential cut
...
Section~\cref{sec:model} briefly describes our mathematical framework and demonstrates its use. Next, we discuss our experimental and software setup in \cref{sec:exp_setup} and then present our experimental data in \cref{sec:exp}.  
 Related work is discussed in ~\cref{sec:related} 
 and  we conclude with  a discussion of implications and future work in \cref{sec:dis}.
