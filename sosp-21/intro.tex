...
Our work seeks to broaden the understanding web-centric applications running under different system structures and the performance-energy trade-offs in this space; furthermore, we are interested in how the mechanisms of slowing down both processor and network request processing affect or contribute to this space. Towards that end, we make the following observations and contributions in our work:

\begin{enumerate}
    \item \textbf{No single strategy for OS to optimize energy and performance:} in both open and closed loop workloads, we find at some request loads it is actually better to use a OS poll than interrupts to improve both performance and energy; these improvements can be as much as 3.3X. (\cref{sec:closed_loop:poll} \cref{sec:mcd:poll}) 
    \item \textbf{Slowing down processing and interrupt delays can reduce energy use in a workload dependent fashion:} we find this is applicable across both OS structures and careful manual control of both delay mechanisms can result in energy savings up to 2X across the workloads and OSes. (\cref{fig:closed_loop_overview}, \cref{fig:mcd_overview}, \cref{fig:mcdsilo_overview})
    \item \textbf{Specializing the software magnifies the energy benefits of slowing down:} we find a specialized OS stack can more aggressively slow down processing to save energy by 73\%, with no noticeable impact on performance, this is attributed to: a) shortened OS path length can get same application work done using fewer instructions, and ii) resultant instruction mix not ALU bound, therefore minimal time penalties with a slowed processor. (~\cref{sec:mcd:slowinos})
    \item \textbf{Specializing the software creates headroom for trade-offs even in application heavy workload:} we find shortened OS paths can both reduce the time spent in the OS processing and improve the application code IPC; this creates additional headroom for both delay mechanisms to ensure additional energy savings between 
    \item \textbf{}
\end{enumerate}
