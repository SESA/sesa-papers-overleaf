...
\node (start) [startstop] {ITR=\textit{n} us};
\node (dec1) [decision, below of=start] {ITR==\textit{0}?};
\node (pro1) [process, right of=dec1, xshift=1.5cm] {ITR-=\textit{2}};
...
\caption{ITR-Delay algorithm flowchart.} \label{fig:itr_delay_flowchart}
\end{figure}