...
%Now to use the model to explore the fo
\begin{figure}
	
\fbox{\tiny \fbox{\begin{minipage}{15em}
1. the use of interrupts for detection and two ITR values to delay detection, \\
	2. an idle policy using a deep sleep state that consumes no energy on the core,\\
	3. for fixed QPS and two different fractions of work time relative the QPS implied arrival rate,\\
	4. and finally over three different instruction mixes with respect to DVFS sensitivity (a) time insensitive and power sensitive, b) time sensitive and power insensitive and c) both time and power sensitive
\end{minipage}}
...
$t_{\text{latency}} = \text{itr} + t_{\text{work}}$
